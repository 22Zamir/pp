...
class Healer(Hero):
    ...
    def __init__(self, name):
        super().__init__(name)
        self.mage_power = self.get_power() * 3
    ...
    def heal(self, target):
        target.set_hp(target.get_hp() + self.__mage_power)
    ...
    def make_a_move(self, friends, enemies):
        ...
        if min_health < 60:
            ...
            if not enemies:
                return
            print(f'Атакую ближнего - {enemies[0].name}')
...
class Tank(Hero):
    ...
    def lower_shied(self):
        if self.shield_up:
            self.shield_up = False
            self.defense /= 2
            self.set_power(self.get_power() * 2)
            print(f'{self.name} спускает щит')
    ...
    def make_a_move(self, friends, enemies):
        ...
        if self.get_hp() > 60:
            self.lower_shied()
            print(f'Атакакую ближнего {enemies[0].name}')
            self.attack(enemies[0])
        else:
            self.raise_shield()
            print(f'Атакую ближнего{enemies[0].name}')
...
class Attacker(Hero):
    ...
    def __init__(self, name):
        super().__init__(name)
        self.coefficient_power = 1

    def attack(self, target):
        target.take_damage(self.get_power() * self.coefficient_power)
        self.power_down()

    def take_damage(self, damage):
        self.set_hp(self.get_hp() - (damage - (self.coefficient_power / 2)))

    def power_up(self):
        self.coefficient_power *= 2
    ...
    def make_a_move(self, friends, enemies):
        ...
        for friend in friends:
            if friend.get_hp() < min_health:
                target_of_power = friends
                min_health = target_of_power.get_hp()

        if min_health < 60:
            print(f'Усиливаю {target_of_power.name}')
            self.power_down()
        else:
            print(f'Атакую - {enemies[0].name}')
            self.attack(enemies[0])
        print('\n')
